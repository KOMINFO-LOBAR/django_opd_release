...
class Command(BaseCommand):
	...
	def create_instansi(self,site,user):
		A='email';timezone=pytz.timezone(_E);table_name='app_settings';app_settings=self.get_data(table_name);mlist=[]
	...
	def create_banner(self,site):
		A='position';timezone=pytz.timezone(_E);table_name='banners';app_model=self.get_data(table_name);mlist=[]
	...
	def create_kategori(self,site):
		timezone=pytz.timezone(_E);table_name='categories';app_model=self.get_data(table_name);mlist=[]
		for i in range(len(app_model)):tmp=app_model[i];obj,created=kategori.objects.update_or_create(slug=tmp[_D],site__id=site.id,defaults={_G:tmp[_C],_A:timezone.localize(tmp[_A]),_B:timezone.localize(tmp[_B])});obj.site.add(site);obj.save();kategori.objects.filter(slug=tmp[_D],site__id=site.id).update(created_at=timezone.localize(tmp[_A]));obj_tag,created=tags.objects.update_or_create(nama=tmp[_C],site__id=site.id,defaults={_D:tmp[_D],_A:timezone.localize(tmp[_A]),_B:timezone.localize(tmp[_B])});obj_tag.site.add(site);obj_tag.save();tags.objects.filter(nama=tmp[_C],site__id=site.id).update(created_at=timezone.localize(tmp[_A]))
	@transaction.atomic
	def create_berita(self,site,user):
		timezone=pytz.timezone(_E);table_name='news';app_model=self.get_data(table_name);cat_news_default,created=kategori.objects.get_or_create(slug='berita-utama',site__id=site.id,defaults={_G:'Berita Utama'});mlist=[]
		for i in range(len(app_model)):
			tmp=app_model[i];obj_photo,created=photo.objects.get_or_create(site_id=site.id,jenis=photo.Jenis.HIGHLIGHT1,file_path=tmp[_H]);sql_='SELECT news.id, news.title, category_news.category_id, categories.name FROM  ';sql_=sql_+'news INNER JOIN  category_news ON news.id = category_news.news_id INNER JOIN ';sql_=sql_+'categories ON category_news.category_id = categories.id WHERE news.slug=';sql_=sql_+'"'+str(tmp[_D])+'"';sql_=sql_+' order by news.id;';app_model_cat=self.get_data(table_name=None,opt_query=sql_);cat_name=[]
			...
			obj.save();obj=berita.objects.filter(judul_seo=tmp[_D],site_id=site.id);obj.update(created_at=timezone.localize(tmp[_A]));obj.update(judul_seo=tmp[_D])
	@transaction.atomic
	def create_related_link(self,site):
		timezone=pytz.timezone(_E);table_name='related_links';app_model=self.get_data(table_name)
	...
	def create_social_media(self,site):
		timezone=pytz.timezone(_E);table_name='social_medias';app_model=self.get_data(table_name)
