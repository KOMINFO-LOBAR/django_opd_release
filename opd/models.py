...
class info_widget(models.Model):
	...
	def save(B,*C,**D):
		if B.publish_date_convert is _E:E=getattr(settings,_K,'UTC');F=pytz.timezone(E);A=parser.parse(B.publish_date);B.publish_date_convert=datetime.datetime(A.year-1,A.month,A.day,A.hour,A.minute,A.second,tzinfo=F)
		super().save(*(C),**D)
