...
class pengumuman(models.Model):
	...
	def save(A,*B,**C):
		if not A.judul_seo:A.judul_seo=uuslug(A.judul,instance=A,slug_field=_E,max_length=255)
...
class artikel(models.Model):
	...
	def save(A,*B,**C):
		if not A.judul_seo:A.judul_seo=uuslug(A.judul,instance=A,slug_field=_E,max_length=250)
...
class info_widget(models.Model):
	...
	def save(B,*C,**D):
		if B.publish_date_convert is _F:E=getattr(settings,'TIME_ZONE','UTC');F=pytz.timezone(E);A=parser.parse(B.publish_date);B.publish_date_convert=datetime.datetime(A.year,A.month,A.day,A.hour,A.minute,A.second,tzinfo=F)
		super().save(*C,**D)
...
class Log(models.Model):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_('site'));kind=models.SmallIntegerField(choices=OptModelKinds.choices,verbose_name=_('kind'));is_need_refresh=models.BooleanField(default=_B);expired=models.DateTimeField()
	class Meta:verbose_name=_('log');verbose_name_plural=_('logs')
	def __str__(A):return f"{A.kind}"
LEN_TITLE=300
LEN_SUB_TITLE=300
class AutoLatestNews(models.Model):
	site=models.ForeignKey(Site,on_delete=models.CASCADE,verbose_name=_('site'));admin=models.ForeignKey(User,on_delete=models.PROTECT);title=models.CharField(_('title'),max_length=LEN_TITLE);sub_title=models.CharField(_('sub title'),max_length=LEN_SUB_TITLE,null=_A,blank=_A);slug=models.SlugField(max_length=255,default='',blank=_A,editable=_B);photo_path=models.CharField(_('photo string'),max_length=1000,null=_A,blank=_A);categories=models.ForeignKey(kategori,on_delete=models.PROTECT,null=_A,blank=_A);kind=models.SmallIntegerField(choices=OptModelKinds.choices,verbose_name=_('kind'));is_editable=models.BooleanField(default=_B);created_at=models.DateTimeField(editable=_B);created_at_str=models.CharField(max_length=30,null=_A,blank=_A)
	class Meta:verbose_name=_(_K);verbose_name_plural=_(_K)
