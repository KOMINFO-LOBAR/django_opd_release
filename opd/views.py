_q="template belum terdaftar, silahkan daftar di halaman <a href='%s'>admin</a>"
_p='template'
_o='-updated_at'
_n=False
_m='num_pages'
...
def get_comment(siteID,newsID,context):
	komentar=comment.objects.filter(site_id=siteID,post_id=newsID,active=_K).order_by(_B).values(_T,_d,_A)
...
def get_tags_result(context,pdata):
	pdata=re.sub('-',_I,pdata);res=berita.objects.filter(tags__nama=pdata).order_by(_B)
	for i in res:i.isi_berita=document_clean(i.isi_berita)
	return res
def document_clean(pdoc):document_test=pdoc;p=re.compile('<.*?>');document_test=p.sub('',document_test);document_test=re.sub('[^\\x00-\\x7F]+',_I,document_test);document_test=re.sub('@\\w+','',document_test);document_test=re.sub('\\s{2,}',_I,document_test);document_test=re.sub('&#;',"'",document_test);return document_test
...
def get_topSection(siteID,context,active_menu):
	namaOPD=Site.objects.filter(id=siteID).values_list(_T,flat=_K)
...
def get_beritaTerbaru(siteID,context,opt):
	...
	model_criteria={_e:OuterRef(_f)};subQry=get_topFoto(model_criteria);BeritaTerbaruLain=_D;BeritaTerbaruAll=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_G,_Z,_F,_R,_a,_H,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:BeritaTerbaruLain=berita.objects.filter(status=Status.PUBLISHED).exclude(site_id=siteID).values(_C,_G,_Z,_F,_R,_a,_H,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
...
def get_beritaTerpopuler(siteID,context,opt):
	...
	model_criteria={_e:OuterRef(_f)};subQry=get_topFoto(model_criteria);BeritaTerpopulerLain=_D;BeritaQry=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED);BeritaCount=BeritaQry.count()
	...
	BeritaTerpopulerAll=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_G,_Z,_F,_R,_a,_H,_A).order_by(_g).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:
		BeritaQry=berita.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED);BeritaCount=BeritaQry.count()
		if BeritaCount==0:BeritaCutOff=0
		else:BeritaCutOff=BeritaQry.order_by(_B).values_list(_C)[min(maxNews,BeritaCount-1)];BeritaCutOff=BeritaCutOff[0]
		BeritaTerpopulerLain=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_G,_Z,_F,_R,_a,_H,_A).order_by(_g).distinct().annotate(foto=subQry)[:maxNews]
...
def get_pengumuman(siteID,context,opt):
	...
	model_criteria={'pengumuman__id':OuterRef('photo__pengumuman__id')};subQry=get_topFoto(model_criteria);PengumumanLain=_D;PengumumanAll=pengumuman.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_G,_F,_b,A,_H,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxPengumuman]
	if opt==1 and PengumumanAll:PengumumanLain=pengumuman.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED).values(_C,_G,_F,_b,A,_H,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxPengumuman]
...
def get_artikel(siteID,context,opd):
	...
	model_criteria={'artikel__id':OuterRef('photo__artikel__id')};subQry=get_topFoto(model_criteria);ArtikelLain=_D;ArtikelAll=artikel.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_G,_F,_b,A,_H,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxArtikel]
	if opd==1 and ArtikelAll:ArtikelLain=artikel.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED).values(_C,_G,_F,_b,A,_H,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxArtikel]
...
def get_beritaKategori(siteID,context,opt,kategori_slug):
	model_criteria={_e:OuterRef(_f)};subQry=get_topFoto(model_criteria);kategoriID=kategori.objects.filter(slug=kategori_slug).first();BeritaKategori=_D
	if kategoriID:BeritaKategori=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED,kategori_id=kategoriID.id).values(_C,_G,_Z,_F,_R,_a,_H,_A).order_by(_B).distinct().annotate(foto=subQry)
...
def get_trending(siteID,context):
	...
	trending=trendingQry.filter(id__gte=trendingCutOff).order_by(_g).values_list(_C,_F,_R)[:maxData];trendingQry=berita.objects.exclude(site_id=siteID);trendingCount=trendingQry.count()
	if trendingCount==0:trendingCutOff=0
	else:trendingCutOff=trendingQry.order_by(_B).values_list(_C)[min(maxData,trendingCount-1)];trendingCutOff=trendingCutOff[0]
	trending_else=trendingQry.filter(id__gte=trendingCutOff).order_by(_g).values_list(_C,_F,_R)[:maxData]
	if trending_else:
...
def get_galeryFoto(siteID,context,opd):
	...
	galeryFoto=galery_foto.objects.filter(site_id=siteID).values(_C,_G,_F,_b,_H,_A,_V).order_by(_B)[:mMax]
...
def get_popup(siteID,context):context['popup']=popup.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_G,_H,_A,'status',_V).order_by(_o)[:1]
...
def get_meta(request,obj,context,jenis):
	J='is_mobile';I='%s://%s%s';H='news_img_meta';G='news_title';F='news_desc';E='%s://%s/%s/%s/';D='news_url';C='news_img';B='\\s+';A='https';context['site_name']='%s://%s'%(A,request.get_host());context['news_type']=jenis if jenis!=_U else'pages';context[J]=request.device[J]
	if jenis==_c:
		context[C]=berita.photo.through.objects.filter(berita__id=obj.id).values(_V);context['news_tags']=berita.tags.through.objects.filter(berita__id=obj.id).values('tags__nama');model_criteria={_e:OuterRef(_f)};subQry=get_topFoto(model_criteria);a=berita.objects.filter(id=obj.id).annotate(foto=subQry)[:1]
...
def get_newsList(request,siteID,context,opt,section,jenis):
	...
	elif section==_h:mList=get_pengumuman(siteID,context,opt)
	elif section==_i:mList=get_artikel(siteID,context,opt)
...
def detail(request,pid,jenis):
	...
	if jenis==_c:
		...
		if request.method==_S:
			...
			if comment_form.is_valid():new_comment,created=comment.objects.get_or_create(site_id=siteID,name__iexact=request.POST.get(_T).strip(),body__iexact=request.POST.get(_d).strip(),defaults={'post':news,_d:request.POST.get(_d),_T:request.POST.get(_T),A:request.POST.get(A)})
		else:comment_form=CommentForm()
		context['new_comment']=new_comment;context['comment_form']=comment_form
	elif jenis==_h:news=get_object_or_404(pengumuman,judul_seo=pid)
	elif jenis==_i:news=get_object_or_404(artikel,judul_seo=pid)
...
def halaman_statis_akses(request,slug):
	...
	news=halaman_statis.objects.filter(menu_id=mFoundID,site_id=siteID).order_by(_o)[:1]
	if not news:news=halaman_statis.objects.create(menu_id=mFoundID,site_id=siteID,judul=mNama,isi_halaman='Informasi tentang "'+mNama+'". Silahkan update di halaman <a target="_blank" href="/dashboard/halaman-statis/">Dashboard.<a>'+'<br><br><br><br>'+'"Halaman ini dibuat otomatis oleh sistem, karena halaman statis untuk menu ini belum dibuat. '+'<br>Silahkan lakukan update di halaman statis."',admin_id=request.user.id);news=halaman_statis.objects.filter(menu_id=mFoundID,site_id=siteID).order_by(_o)[:1]
	news=news.get();optID=3;jenis=_U;get_topSection(siteID,context,_c);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_meta(request,news,context,jenis)
	if news:news.created_at=get_natural_datetime(news.created_at)
...
def dokumen_ajax(request,idx):
	...
	if idx==0:
		object_list=dokumen.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_Y,B,_P,A,_Q).order_by(_B)[:max];n=object_list.count()
		if max-n>0:object_list.union(dokumen.objects.exclude(site_id=siteID,status=Status.PUBLISHED).values(_C,_Y,B,_P,A,_Q).order_by(_B)[:max-n])
	elif idx==1:object_list=dokumen.objects.exclude(site_id=siteID,status=Status.PUBLISHED).values(_C,_Y,B,_P,A,_Q).order_by(_B)[:max]
	elif idx==2:object_list=dokumen.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_Y,B,_P,A,_Q).order_by(_B)
