...
def get_siteID(request):
	if request:siteID=Site.objects.filter(domain=request.get_host()).values_list(_C,flat=_E);return siteID[0]if siteID else 0
	return 0
def get_comment(siteID,newsID,context):
	komentar=comment.objects.filter(site_id=siteID,post_id=newsID,active=_E).order_by(_B).values(_S,_c,_A)
...
def get_topSection(siteID,context,active_menu):
	namaOPD=Site.objects.filter(id=siteID).values_list(_S,flat=_E)
	if namaOPD.count()>0:context['namaOPD']=namaOPD[0]
	mObjMenu=menus.ClsMenus();myMenu=menus.ClsMenus(siteID,_k);context[_T]=myMenu.get_menus();context['activeMenuList']=myMenu.find_activeMenuList(active_menu);context['socialMedia']=social_media.objects.filter(site_id=siteID);logoTop=logo.objects.filter(site_id=siteID,position=logo.Position.TOP)
...
def get_beritaTerbaru(siteID,context,opt):
	...
	model_criteria={_l:OuterRef(_m)};subQry=get_topFoto(model_criteria);BeritaTerbaruLain=_D;BeritaTerbaruAll=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_H,_a,_G,_Q,_b,_I,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:BeritaTerbaruLain=berita.objects.filter(status=Status.PUBLISHED).exclude(site_id=siteID).values(_C,_H,_a,_G,_Q,_b,_I,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
...
def get_beritaTerpopuler(siteID,context,opt):
	...
	model_criteria={_l:OuterRef(_m)};subQry=get_topFoto(model_criteria);BeritaTerpopulerLain=_D;BeritaQry=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED);BeritaCount=BeritaQry.count()
	...
	BeritaTerpopulerAll=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_H,_a,_G,_Q,_b,_I,_A).order_by(_d).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:
		BeritaQry=berita.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED);BeritaCount=BeritaQry.count()
		if BeritaCount==0:BeritaCutOff=0
		else:BeritaCutOff=BeritaQry.order_by(_B).values_list(_C)[min(maxNews,BeritaCount-1)];BeritaCutOff=BeritaCutOff[0]
		BeritaTerpopulerLain=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_H,_a,_G,_Q,_b,_I,_A).order_by(_d).distinct().annotate(foto=subQry)[:maxNews]
...
def get_pengumuman(siteID,context,opt):
	...
	model_criteria={'pengumuman__id':OuterRef('photo__pengumuman__id')};subQry=get_topFoto(model_criteria);PengumumanLain=_D;PengumumanAll=pengumuman.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_H,_G,_U,A,_I,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxPengumuman]
	if opt==1 and PengumumanAll:PengumumanLain=pengumuman.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED).values(_C,_H,_G,_U,A,_I,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxPengumuman]
...
def get_artikel(siteID,context,opd):
	...
	model_criteria={'artikel__id':OuterRef('photo__artikel__id')};subQry=get_topFoto(model_criteria);ArtikelLain=_D;ArtikelAll=artikel.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_H,_G,_U,A,_I,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxArtikel]
	if opd==1 and ArtikelAll:ArtikelLain=artikel.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED).values(_C,_H,_G,_U,A,_I,_A).order_by(_B).distinct().annotate(foto=subQry)[:maxArtikel]
...
def get_beritaKategori(siteID,context,opt,kategori_slug):
	model_criteria={_l:OuterRef(_m)};subQry=get_topFoto(model_criteria);kategoriID=kategori.objects.filter(slug=kategori_slug).first();BeritaKategori=_D
	if kategoriID:BeritaKategori=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED,kategori_id=kategoriID.id).values(_C,_H,_a,_G,_U,_b,_I,_A).order_by(_B).distinct().annotate(foto=subQry)
...
def get_hitCounter(request,obj,content_type):
	...
	if content_type_id:
		hit_update=HitCount.objects.filter(object_pk=obj.id,content_type_id=content_type_id.id).values_list('hits',flat=_E)
...
def get_trending(siteID,context):
	...
	trending=trendingQry.filter(id__gte=trendingCutOff).order_by(_d).values_list(_C,_G,_Q)[:maxData];trendingQry=berita.objects.exclude(site_id=siteID);trendingCount=trendingQry.count()
	if trendingCount==0:trendingCutOff=0
	else:trendingCutOff=trendingQry.order_by(_B).values_list(_C)[min(maxData,trendingCount-1)];trendingCutOff=trendingCutOff[0]
	trending_else=trendingQry.filter(id__gte=trendingCutOff).order_by(_d).values_list(_C,_G,_Q)[:maxData]
	if trending_else:
...
def get_galeryFoto(siteID,context,opd):
	...
	galeryFoto=galery_foto.objects.filter(site_id=siteID).values(_C,_H,_G,_U,_I,_A,_V).order_by(_B)[:mMax]
...
def get_popup(siteID,context):context['popup']=popup.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_H,_I,_A,'status',_V).order_by(_n)[:1]
def get_galeryVideo(siteID,context,opd):
	A='galeryVideo'
	if opd==1:mMax=6
	elif opd==2:mMax=50
	galeryVideo=galery_video.objects.filter(site_id=siteID).values(_C,_H,_G,_I,_A,'embed','embed_video').order_by(_B)[:mMax]
...
def get_meta(request,obj,context,jenis):
	...
	if jenis==_W:
		...
		for i in a:context[C]=D%(A,request.get_host(),_W,i.slug);context[E]=Truncator(strip_tags(i.isi_berita)).words(30).strip();context[F]=Truncator(strip_tags(i.judul)).words(15).strip()
		news_img_meta=berita.photo.through.objects.filter(berita__id=obj.id).order_by('photo__jenis')
		if news_img_meta.count()>0:context[G]=H%(A,request.get_host(),news_img_meta[0].photo);options={_K:(100,100),'crop':_E}
	elif jenis==_e:
		context[B]=pengumuman.photo.through.objects.filter(pengumuman__id=obj.id).values(_V);a=pengumuman.objects.filter(id=obj.id)
		for i in a:context[C]=D%(A,request.get_host(),_e,i.judul_seo);context[E]=Truncator(strip_tags(i.isi_pengumuman)).words(30).strip();context[F]=Truncator(strip_tags(i.judul)).words(15).strip()
		...
		for i in a:context[C]=D%(A,request.get_host(),_f,i.judul_seo);context[E]=Truncator(strip_tags(i.isi_artikel)).words(30).strip();context[F]=Truncator(strip_tags(i.judul)).words(15).strip()
...
def get_newsList(request,siteID,context,opt,section,jenis):
	...
	if mList is not _D:
		paginator=Paginator(mList,news_per_page);page_number=request.GET.get(_g)
		if page_number is _D:page_number=1
		if page_number:context[_h]=paginator.get_page(page_number);context[_i]=paginator.page_range;context[_j]=paginator.num_pages
...
def index(request):
	...
	if not siteID:context[_L]=_N%(request.get_host(),_J);return render(request,_M,context)
	context[_O]='index';active_menu='beranda';optID=1
	if request.method==_R:
		pdata=request.POST.get(_F)
		if pdata:
			pdata=pdata.strip()
			if pdata!='':return redirect(_X+slugify(pdata)+_Y)
	get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_middleSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_trending(siteID,context);get_banner_all(siteID,context);get_beritaTerbaru(siteID,context,optID);get_pengumuman(siteID,context,optID);get_artikel(siteID,context,optID);get_galeryLayanan(siteID,context);statistik=get_statistic(siteID,_E);context.update(statistik);get_popup(siteID,context);get_weather(context);template_name=get_template(siteID);print('TEMPLATENAME',template_name);response=render(request,f"{template_name}index.html",context);response.set_cookie(key=_S,value='my_value',samesite='None',secure=_E);return response
class PostMixinDetailView:
	model=berita;template_name='magnews2-master/detail.html';slug=_D
	def get(self,request,*args,**kwargs):self.slug=kwargs[_Q];return super(PostMixinDetailView,self).get(request,*(args),**kwargs)
	def get_context_data(self,**kwargs):context=super(PostMixinDetailView,self).get_context_data(**kwargs);context.update(detail(self.request,self.slug,_W));return context
class PostDetailView(PostMixinDetailView,HitCountDetailView):count_hit=_E;qs=Hit.objects.all();do_summary(qs)
def detail(request,pid,jenis):
	A='email';context={};siteID=get_siteID(request)
	if siteID==0:context[_L]=_N%(request.get_host(),_J);return render(request,_M,context)
	if request.method==_R:
		pdata=request.POST.get(_F)
		if pdata:
			pdata=pdata.strip()
	...
	if jenis==_W:
		...
		if mList is not _D:
			paginator=Paginator(mList,news_per_page);page_number=request.GET.get(_g)
			if page_number is _D:page_number=1
			context[_h]=paginator.get_page(page_number);context[_i]=paginator.page_range;context[_j]=paginator.num_pages
		new_comment=_D
		if request.method==_R:
	...
	elif jenis==_e:news=get_object_or_404(pengumuman,judul_seo=pid)
	elif jenis==_f:news=get_object_or_404(artikel,judul_seo=pid)
	...
	optID=3;context[_O]=jenis;context['news']=news;get_topSection(siteID,context,jenis);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_meta(request,news,context,jenis);statistik=get_statistic(siteID,_E);context.update(statistik);template_name=get_template(siteID);return context
def detail_list(request,section,jenis):
	...
	if request.method==_R:
		...
		if pdata:
			pdata=pdata.strip()
			if pdata!='':return redirect(_X+slugify(pdata)+_Y)
	get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_newsList(request,siteID,context,optID,section,jenis);statistik=get_statistic(siteID,_E);context.update(statistik);template_name=get_template(siteID);return render(request,f"{template_name}detail-list.html",context)
def search_result(request,pdata):
	context={};siteID=get_siteID(request)
	if siteID==0:context[_L]=_N%(request.get_host(),_J);return render(request,_M,context)
	optID=2;context[_O]=_F;active_menu=_F
	if request.method==_R:
		pdata=request.POST.get(_F)
		if pdata:
			pdata=pdata.strip()
			if pdata!='':return redirect(_X+slugify(pdata)+_Y)
	get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);statistik=get_statistic(siteID,_E);context.update(statistik);context[_F]=pdata;mList=get_search_result(context,pdata);news_per_page=5
	if mList is not _D:
		paginator=Paginator(mList,news_per_page);page_number=request.GET.get(_g)
		if page_number is _D:page_number=1
		context[_h]=paginator.get_page(page_number);context[_i]=paginator.page_range;context[_j]=paginator.num_pages
...
def tags_result(request,pdata):
	...
	if request.method==_R:
		...
		if pdata:
			pdata=pdata.strip()
			if pdata!='':return redirect(_X+slugify(pdata)+_Y)
	get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);statistik=get_statistic(siteID,_E);context.update(statistik);context[_F]=pdata;mList=get_tags_result(context,pdata);news_per_page=5
	if mList is not _D:
		paginator=Paginator(mList,news_per_page);page_number=request.GET.get(_g)
		if page_number is _D:page_number=1
		context[_h]=paginator.get_page(page_number);context[_i]=paginator.page_range;context[_j]=paginator.num_pages
...
def halaman_statis_akses(request,slug):
	...
	for i in menu_id:print(_T,i.id,i.nama)
	mFoundID=0;mNama=''
	for i in menu_id:
	...
	news=news.get();optID=3;jenis=_T;get_topSection(siteID,context,_W);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_meta(request,news,context,jenis);statistik=get_statistic(siteID,_E);context.update(statistik)
	if news:news.created_at=get_natural_datetime(news.created_at)
...
def dokumen_ajax(request,idx):
	...
	if idx==0:
		object_list=dokumen.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_Z,B,_K,A,_P).order_by(_B)[:max];n=object_list.count()
		if max-n>0:object_list.union(dokumen.objects.exclude(site_id=siteID,status=Status.PUBLISHED).values(_C,_Z,B,_K,A,_P).order_by(_B)[:max-n])
	elif idx==1:object_list=dokumen.objects.exclude(site_id=siteID,status=Status.PUBLISHED).values(_C,_Z,B,_K,A,_P).order_by(_B)[:max]
	elif idx==2:object_list=dokumen.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_Z,B,_K,A,_P).order_by(_B)
	if idx==0 or idx==1 or idx==2:
		for i in object_list:i[_P]=get_natural_datetime(i[_P]);i[_K]=naturalsize(i[_K]);i[A]=settings.MEDIA_URL+i[A]
		mData=list(object_list)
	else:mData=_D
	return JsonResponse(mData,safe=_k)
def satudata_ajax(request,idx):
	if idx==0 or idx==1:
		for i in object_list:i[_P]=get_natural_datetime(i[_P]);i[_K]=naturalsize(i[_K])
		mData=list(object_list)
	else:mData=_D
	return JsonResponse(mData,safe=_k)
def satudata_result(request):
	context={};siteID=get_siteID(request)
	if siteID==0:context[_L]=_N%(request.get_host(),_J);return render(request,_M,context)
	optID=2;context[_O]=_F;active_menu=_F;get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);statistik=get_statistic(siteID,_E);context.update(statistik);template_name=get_template(siteID);return render(request,f"{template_name}/satudata-result.html",context)
def dokumen_result(request):
	context={};siteID=get_siteID(request)
	if siteID==0:context[_L]=_N%(request.get_host(),_J);return render(request,_M,context)
	optID=2;context[_O]=_F;active_menu=_F;get_topSection(siteID,context,active_menu);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);statistik=get_statistic(siteID,_E);context.update(statistik);template_name=get_template(siteID);return render(request,f"{template_name}/dokumen-result.html",context)
def get_template_id(site_id,is_frontend=_E):
	print('site',site_id);template=Template.objects.filter(site__id=site_id,is_frontend=is_frontend).values_list(_C,flat=_E)[:1];print(_o,template)
	if template:return template[0]
	raise Http404(_p%_J)
def get_template(site_id,is_frontend=_E):
	print('site',site_id);template=Template.objects.filter(site__id=site_id,is_frontend=is_frontend).values_list('rel_path',flat=_E)[:1];print(_o,template)
	if template:return template[0]
	raise Http404(_p%_J)