...
def get_comment(siteID,newsID,context):
	komentar=comment.objects.filter(site_id=siteID,post_id=newsID,active=_E).order_by(_B).values(_S,_d,_A)
...
def get_beritaTerbaru(siteID,context,opt):
	...
	model_criteria={_k:OuterRef(_l)};subQry=get_topFoto(model_criteria);BeritaTerbaruLain=_D;BeritaTerbaruAll=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED).values(_C,_H,_Z,_G,_I,_a,_J,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:BeritaTerbaruLain=berita.objects.filter(status=Status.PUBLISHED).exclude(site_id=siteID).values(_C,_H,_Z,_G,_I,_a,_J,_A,A).order_by(_B).distinct().annotate(foto=subQry)[:maxNews]
...
def get_beritaTerpopuler(siteID,context,opt):
	...
	BeritaTerpopulerAll=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_H,_Z,_G,_I,_a,_J,_A).order_by(_e).distinct().annotate(foto=subQry)[:maxNews]
	if opt==1:
		BeritaQry=berita.objects.exclude(site_id=siteID).filter(status=Status.PUBLISHED);BeritaCount=BeritaQry.count()
		if BeritaCount==0:BeritaCutOff=0
		else:BeritaCutOff=BeritaQry.order_by(_B).values_list(_C)[min(maxNews,BeritaCount-1)];BeritaCutOff=BeritaCutOff[0]
		BeritaTerpopulerLain=BeritaQry.filter(id__gte=BeritaCutOff).values(_C,_H,_Z,_G,_I,_a,_J,_A).order_by(_e).distinct().annotate(foto=subQry)[:maxNews]
...
def get_beritaKategori(siteID,context,opt,kategori_slug):
	...
	if kategoriID:BeritaKategori=berita.objects.filter(site_id=siteID,status=Status.PUBLISHED,kategori_id=kategoriID.id).values(_C,_H,_Z,_G,_I,_a,_J,_A).order_by(_B).distinct().annotate(foto=subQry)
...
def get_trending(siteID,context):
	...
	trending=trendingQry.filter(id__gte=trendingCutOff).order_by(_e).values_list(_C,_G,_I)[:maxData];trendingQry=berita.objects.exclude(site_id=siteID);trendingCount=trendingQry.count()
	if trendingCount==0:trendingCutOff=0
	else:trendingCutOff=trendingQry.order_by(_B).values_list(_C)[min(maxData,trendingCount-1)];trendingCutOff=trendingCutOff[0]
	trending_else=trendingQry.filter(id__gte=trendingCutOff).order_by(_e).values_list(_C,_G,_I)[:maxData]
	if trending_else:
...
def get_newsList(request,siteID,context,opt,section,jenis):
	...
	elif section==_b:mList=get_pengumuman(siteID,context,opt)
	elif section==_c:mList=get_artikel(siteID,context,opt)
...
def detail(request,pid,jenis):
	...
	if jenis==_V:
		...
		if request.method==_R:
			...
			if comment_form.is_valid():new_comment,created=comment.objects.get_or_create(site_id=siteID,name__iexact=request.POST.get(_S).strip(),body__iexact=request.POST.get(_d).strip(),defaults={'post':news,_d:request.POST.get(_d),_S:request.POST.get(_S),A:request.POST.get(A)})
		else:comment_form=CommentForm()
		context['new_comment']=new_comment;context['comment_form']=comment_form
	elif jenis==_b:news=get_object_or_404(pengumuman,judul_seo=pid);get_hitCounter(request,news,_b)
	elif jenis==_c:news=get_object_or_404(artikel,judul_seo=pid);get_hitCounter(request,news,_c)
...
def halaman_statis_akses(request,slug):
	...
	news=news.get();optID=3;jenis=_T;get_hitCounter(request,news,'halaman_statis');get_topSection(siteID,context,_V);get_bottomSection(siteID,context,optID);get_sideBar(siteID,context,optID);get_meta(request,news,context,jenis);statistik=get_statistic(siteID,_E);context.update(statistik)
	if news:news.created_at=get_natural_datetime(news.created_at)
...
def get_template_id(site_id,is_frontend=_E):
	print(_n,site_id);template=Template.objects.filter(site__id=site_id,is_frontend=is_frontend).values_list(_C,flat=_E)[:1];print(_o,template)
	if template:return template[0]
	raise Http404(_p%_K)
def get_template(site_id,is_frontend=_E):
	print(_n,site_id);template=Template.objects.filter(site__id=site_id,is_frontend=is_frontend).values_list('rel_path',flat=_E)[:1];print(_o,template)
	if template:return template[0]
	raise Http404(_p%_K)