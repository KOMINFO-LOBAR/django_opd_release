...
def get_size_format(b,factor=1024,suffix='B'):
	...
	for C in['','K','M','G','T','P','E','Z']:
		...
		b/=A
	return f"{b:.2f}Y{B}"
def compress_img(image_name,new_size_ratio=.9,quality=90,width=None,height=None,replace=True):
	...
	if D<1.:A=A.resize((int(A.size[0]*D),int(A.size[1]*D)),Image.LANCZOS);print(J,A.size)
...
if __name__=='__main__':
	import argparse;parser=argparse.ArgumentParser(description='Simple Python script for compressing and resizing images');parser.add_argument('image',help='Target image to compress and/or resize');parser.add_argument('-j','--to-jpg',action='store_true',help='Whether to convert the image to the JPEG format');parser.add_argument('-q','--quality',type=int,help='Quality ranging from a minimum of 0 (worst) to a maximum of 95 (best). Default is 90',default=90);parser.add_argument('-r','--resize-ratio',type=float,help='Resizing ratio from 0 to 1, setting to 0.5 will multiply width & height of the image by 0.5. Default is 1.0',default=1.);parser.add_argument('-w','--width',type=int,help='The new width image, make sure to set it with the `height` parameter');parser.add_argument('-hh','--height',type=int,help='The new height for the image, make sure to set it with the `width` parameter');args=parser.parse_args();print('='*50);print('[*] Image:',args.image);print('[*] Quality:',args.quality);print('[*] Resizing ratio:',args.resize_ratio)
