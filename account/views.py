...
def social_media(request,mode='',pk=''):
	...
	if C==_M:
		if A.method==_I:
			...
			if B.is_valid():
				if models.social_media.objects.filter(site_id=E,link=A.POST.get(_f)).exists():messages.info(A,mMsgBox.get(_v,A.POST.get(_f)))
				else:
					L=models.social_media.objects.create(site_id=E,jenis=A.POST.get(_q),link=A.POST.get(_f))
					if L:messages.info(A,mMsgBox.get(_X,A.POST.get(_q)))
					return redirect(H)
		...
		if A.method==_I:
			...
			if B.is_valid():D.save();messages.info(A,mMsgBox.get(_P,A.POST.get(_q)));return redirect(H)
...
def logo(request,mode='',pk=''):
	...
	if mode==_M:
		if A.method==_I:
			...
			I=A.POST.get(_AB)
			if I:
				L=I.replace(_r,'');M,N=models.photo.objects.update_or_create(site_id=B,jenis=H,defaults={_m:L});Q,R=models.logo.objects.update_or_create(site_id=B,position=A.POST.get(C),defaults={_AC:M.id})
...
def banner(request,mode='',pk=''):
	...
	if mode==_M:
		if A.method==_I:
			...
			if H:
				K=H.replace(_r,'');L,M=models.photo.objects.update_or_create(site_id=C,jenis=D,defaults={_m:K});P,Q=models.banner.objects.update_or_create(site_id=C,position=B,defaults={_AC:L.id,_f:A.POST.get('banner-link')})
	...
	I=J;N=models.menu.objects.filter(nama=I,is_admin_menu=_B);O={_K:E.get_menus(),_Q:E.create_breadCrumb(I),_R:E.find_activeMenuList(I),_V:mode,_U:F,_AD:G,_S:get_namaOPD(C),_T:N};return render(A,'account/banner.html',O)
...
def save_photo_slideshow(idx,site_id,photo_id,str_foto_path):
	C=photo_id;B=site_id;print('Begin INSPECT');D=get_photo_kind(idx);print('mode add/edit PHOTO IDs == ');print(C);E=str_foto_path;print('str_foto_path_replace');print(E);print(_q);print(D);print('siteid');print(B)
...
def berita(request,mode='',pk='',photoID=''):
	...
	elif G==_M:J=formset_factory(forms.PhotoForm,extra=3)
	if G==_M:
		if A.method==_I:
			...
			if D.is_valid():
				...
				print('isi berita =',E);S=models.berita.objects.create(site_id=F,judul=C,admin_id=A.user.id,kategori_id=A.POST.get('kategori'),isi_berita=E,status=A.POST.get(_d));T=A.POST.getlist('tags');save_tags(T,S);B=0
				for a in H:
					K=A.POST.get(_L+str(B)+_b)
					if K:
						U=A.POST.get(_L+str(B)+_c);P=save_photo_slideshow(B,F,U,K)
						if P:S.photo.add(P);print(_g+str(B))
					B+=1
		...
		if A.method==_I:
			...
			if D.is_valid():
				...
				for a in H:
					K=A.POST.get(_L+str(B)+_b)
					if K!='':U=A.POST.get(_L+str(B)+_c);P=save_photo_slideshow(B,F,U,K);I.photo.add(P);print(_g+str(B))
					B+=1
	...
	V=_i;b=models.menu.objects.filter(nama=V,is_admin_menu=_B);c={_K:Q.get_menus(),_Q:Q.create_breadCrumb(V),_R:Q.find_activeMenuList(V),_V:G,_U:D,_p:H,_S:get_namaOPD(F),_T:b};return render(A,'account/berita.html',c)
@login_required(login_url=_O)
...
def pejabat(request,mode='',pk=''):
	...
	I='pejabat';U=models.menu.objects.filter(nama=I,is_admin_menu=_B);V={_K:F.get_menus(),_Q:F.create_breadCrumb(I),_R:F.find_activeMenuList(I),_V:C,_U:D,_AD:G,_S:get_namaOPD(B),_T:U};return render(A,'account/pejabat.html',V)
...
def dokumen(request,mode='',pk=''):
		...
		if A.method==_I:
			...
			if C.is_valid():
				...
				if models.dokumen.objects.filter(site_id=E,nama__iexact=A.POST.get(_E)).exists():messages.info(A,mMsgBox.get(_v,A.POST.get(_E)))
					...
					B=models.dokumen.objects.create(site_id=E,nama=A.POST.get(_E),admin_id=A.user.id,file_path=M,deskripsi=A.POST.get(_y),status=A.POST.get(_d));B.size=os.stat(B.file_path.path).st_size;B.save()
	...
	H='dokumen';N=models.menu.objects.filter(nama=H,is_admin_menu=_B);O={_K:G.get_menus(),_Q:G.create_breadCrumb(H),_R:G.find_activeMenuList(H),_V:D,_U:C,_S:get_namaOPD(E),_T:N};return render(A,'account/dokumen.html',O)
@login_required(login_url=_O)
		...
		if A.method==_I:
			...
			if D.is_valid():
				if models.halaman_statis.objects.filter(site_id=C,menu_id=A.POST.get(_K)).count()>1:messages.info(A,mMsgBox.get('menu_already_exists'))
					...
					G,Z=models.halaman_statis.objects.update_or_create(site_id=C,menu_id=A.POST.get(_K),defaults={_s:C,_F:A.POST.get(_F),_z:A.POST.get(_z),'admin_id':A.user.id,'is_edited':_B});B=0
					for W in F:
	...
	if C==_M:
		if A.method==_I:
			...
			for X in F:
				...
				if G:N=A.POST.get(_L+str(B)+_c);O=save_photo_slideshow(B,D,N,G);print(_g+str(B));a=models.galery_foto.objects.create(site_id=D,judul=A.POST.get(_F),admin_id=A.user.id,photo_id=O.id)
				B+=1
			return redirect(Q)
	...
	if D==_M:
		if A.method==_I:
			...
			for a in F:
				...
				if I:
					K=I.replace(_r,'');L=models.photo.Jenis.POPUP
		...
		if A.method==_I:
			...
			if E.is_valid():
				...
				for a in F:
					I=A.POST.get(_L+str(C)+_b)
					if I:
						K=I.replace(_r,'');L=models.photo.Jenis.POPUP;U=A.POST.get(_L+str(C)+_c)
						if U:M,b=models.photo.objects.update_or_create(id=U,defaults={_s:B,_q:L,_m:K})
		...
		if A.method==_I:
			...
			if B.is_valid():
				...
				for b in E:
					...
					if J:
						L=J.replace(_r,'');M=models.photo.Jenis.BANNER_ALL;N=models.photo.objects.create(site_id=G,jenis=M,file_path=L);Q=B.cleaned_data.get('site');F=B.save(commit=_C);F.photo_id=N.id;F.save()
		...
		if A.method==_I:
			...
			if B.is_valid():
				...
				for b in E:
					J=A.POST.get(_L+str(C)+_b)
					if J:
						L=J.replace(_r,'');M=models.photo.Jenis.BANNER_ALL;W=A.POST.get(_L+str(C)+_c)
						if W:N,c=models.photo.objects.update_or_create(id=W,defaults={_s:G,_q:M,_m:L})
...
def social_media_ajax(request):
	C=get_siteID(request);A=models.social_media.objects.filter(site_id=C).values(_A,_q,_f,_D)
...
def banner_ajax(request):
	C=get_siteID(request);A=models.banner.objects.filter(site_id=C).values(_A,'position',_t,_f,_D)
	for B in A:B[_D]=get_natural_datetime(B[_D])
	D=list(A);return JsonResponse(D,safe=_C)
def menu_ajax(request):
	C=get_siteID(request);A=models.menu.objects.filter(site__id=C,is_admin_menu=_C,is_master_menu=_C).values(_A,_E,'href','icon',_u,_D).order_by(_n,_w)
	for B in A:B[_D]=get_natural_datetime(B[_D])
	D=list(A);return JsonResponse(D,safe=_C)
def menu_statis_ajax(request):A=get_siteID(request);B=models.menu.objects.filter(site__id=A,is_statis_menu=_B,is_admin_menu=_C).exclude(href='#').order_by(_n,_w);C=serializers.serialize('json',B,fields=(_A,_E));return HttpResponse(C,content_type='application/json')
def berita_ajax(request):
	C='photo__berita__id';D=get_siteID(request);E={'berita__id':OuterRef(C)};B=models.berita.objects.filter(site_id=D).values(_A,_F,_x,'kategori__nama',_d,_D).distinct().annotate(foto=get_topFoto(E)).annotate(foto_count=Count(C))
...
def dokumen_ajax(request):
	C='size';B=request;E=get_siteID(B);D=models.dokumen.objects.filter(site_id=E).values(_A,_E,_y,C,_m,_D)
	for A in D:A[_D]=get_natural_datetime(A[_D]);A[C]=naturalsize(A[C]);A[_m]=Truncator(A[_m]).chars(30);A['extra_field']='%s://%s%s%s'%(B.scheme,B.get_host(),settings.MEDIA_URL,A[_m])
	F=list(D);return JsonResponse(F,safe=_C)
def agenda_ajax(request):
	C=get_siteID(request);B=models.agenda.objects.filter(site_id=C).values(_A,_E,_y,'lokasi',_AH,'jam','penyelenggara','dihadiri_oleh',_d,_D)
...
def popup_ajax(request):
	C=get_siteID(request);A=models.popup.objects.filter(site_id=C).values(_A,_F,_t,_d,_D)
	for B in A:B[_D]=get_natural_datetime(B[_D])
	D=list(A);return JsonResponse(D,safe=_C)
def komentar_ajax(request):
	A='created_at';D=get_siteID(request);B=models.comment.objects.filter(site_id=D).values(_A,_l,_A1,'body','post__judul',A,'active')
	for C in B:C[A]=get_natural_datetime(C[A])
	E=list(B);return JsonResponse(E,safe=_C)
def galery_layanan_ajax(request):
	C=get_siteID(request);B=models.galery_layanan.objects.filter(site_id=C).values(_A,_F,_d,_t,_D)
	for A in B:A[_D]=get_natural_datetime(A[_D]);A[_F]=Truncator(A[_F]).words(5)
	D=list(B);return JsonResponse(D,safe=_C)
...
def banner_all_ajax(request):
	D=get_siteID(request);A=models.banner_all.objects.all().values(_A,_l,_f,_t,_d,_D)
	for B in A:B[_D]=get_natural_datetime(B[_D])
	C=list(A);return JsonResponse(C,safe=_C)
...
def site_activity(request):
	...
	for A in G:
		...
		K={_A:A[_A],_N:A[_N],_A7:C,_o:E,_A0:F};D.append(K)
...
def site_activity_pie_chart(request):
	...
	for A in P:
		...
		Q+=F;U={_A:A[_A],_N:A[_N],_A7:E,_o:H,_A0:F};C.append(U)
	...
	for A in C:
		...
		if J<I:
			...
			else:K+=A[_o];L+=A[_A7];M+=A[_A0];O+=1
		else:K+=A[_o];L+=A[_A7];M+=A[_A0];O+=1
...
def site_activity_detail(request,siteID):
	...
	for A in F:
		...
		if D:
			for G in D:E={_A:A[_A],_n:A[_u],_K:A[_E],_F:G.judul,_o:1}
		else:E={_A:A[_A],_n:A[_u],_K:A[_E],_F:'',_o:0}
		C.append(E)
...
def site_activity_detail_pie_chart(request,siteID):
	...
	for A in I:
		...
		if G:
			for J in G:H={_A:A[_A],_n:A[_u],_K:A[_E],_F:J.judul,_o:1}
		else:H={_A:A[_A],_n:A[_u],_K:A[_E],_F:'',_o:0}
		B.append(H)
	print('begin create pie chart data');E=len(B);print(E);C=len(list(filter(lambda x:x[_o]==0,B)));print(C);D=len(list(filter(lambda x:x[_o]==1,B)));print(D);C=C/E*100;D=D/E*100;K=[{_l:'Menu Terisi','y':D,'sliced':_B,'selected':_B},{_l:'Menu Kosong','y':C}];return JsonResponse(K,safe=_C)
...
def site_kontribusi_kuantitas_pie_chart(request,kategori_id):
	...
	if J==1:L=list(models.Site.objects.exclude(id=1).order_by(_N).values(_A,_N))
		...
		S=list(models.instansi.objects.filter(kategori_id=J).values(_s))
		for A in S:K.append(A[_s])
...
def site_kontribusi_kualitas_pie_chart(request,kategori_id):
	...
	if J==1:M=list(models.Site.objects.exclude(id=1).order_by(_N).values(_A,_N))
		...
		U=list(models.instansi.objects.filter(kategori_id=J).values(_s))
		for A in U:K.append(A[_s])
